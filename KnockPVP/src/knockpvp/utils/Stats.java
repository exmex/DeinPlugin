...
public class Stats {
	...
	public static File file = new File("plugins//KnockPvP//stats.yml");
	...
	public static void loadStatsFromHashMapIntoConfig(Player p){
		cfg.set(p.getUniqueId() + ".Kills", kills.get(p.getUniqueId().toString()));
		cfg.set(p.getUniqueId() + ".Deaths", deaths.get(p.getUniqueId().toString()));
		cfg.set(p.getUniqueId() + ".Coins", coins.get(p.getUniqueId().toString()));
		try {
			cfg.save(file);
		} catch (IOException e) {
			// TODO Auto-generated catch block
